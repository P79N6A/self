...
#   修改版
#   路径：/root/ssrmu
#   init.d:/etc/init.d/ssrmu
#   python server-ssrmu.py
#=================================================

sh_ver="1.0.25.1"
filepath=$(cd "$(dirname "$0")"; pwd)
file=$(echo -e "${filepath}"|awk -F "$0" '{print $1}')
ssr_folder="/root/ssrmu"
...
check_pid(){
    PID=`ps -ef |grep -v grep | grep server-ssrmu.py |awk '{print $2}'`
}
check_crontab(){
...
Download_SSR(){
    ...
    unzip "manyuser.zip"
    [[ ! -e "/root/shadowsocksr-manyuser/" ]] && echo -e "${Error} ShadowsocksR服务端 解压失败 !" && rm -rf manyuser.zip && exit 1
    mv "/root/shadowsocksr-manyuser/" "/root/ssrmu/"
    cp "/root/ssrmu/server.py" "/root/ssrmu/server-ssrmu.py"
    [[ ! -e "/root/ssrmu/" ]] && echo -e "${Error} ShadowsocksR服务端 重命名失败 !" && rm -rf manyuser.zip && rm -rf "/usr/local/shadowsocksr-manyuser/" && exit 1
    rm -rf manyuser.zip
    cd "ssrmu"
...
Service_SSR(){
    if [[ ${release} = "centos" ]]; then
        if ! wget --no-check-certificate https://raw.githubusercontent.com/max2max/self/master/linux/shell/ssrmu -O /etc/init.d/ssrmu; then
